"""Command-line interface for the anonymization tool."""
import argparse
import sys
import os
...
def gen_passphrase():
    ...
    import secrets
    wordlist = [
        "river", "mountain", "forest", "ocean", "sunset", "tree", "rock", "stone",
        "wind", "fire", "earth", "sky", "star", "moon", "cloud", "rain", "snow",
        "ice", "gold", "silver", "crystal", "flower", "leaf", "grass", "sand"
    ]
    # Select 4 random words
    selected = secrets.choice(wordlist), secrets.choice(wordlist), secrets.choice(wordlist), secrets.choice(wordlist)
...
def cmd_anonymize(args):
    ...
    if args.verbose:
        token_count = len(result["mapping"]) if isinstance(result["mapping"], dict) else 0
        print(f"Anonymized {token_count} entities", file=sys.stderr)
...
def cmd_deanonymize(args):
    ...
    # Create deanonymizer
    deanonymizer = Deanonymizer(mapping=mapping, passphrase=args.passphrase)
...
def main():
    ...
    gen_parser.set_defaults(func=lambda args: print(gen_passphrase()))
